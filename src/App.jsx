...
function PickChart({ history, width=340, height=160, t }) {
  ...
  const pts=history.map((p,i)=>({x:pad.l+(i/(history.length-1))*w,y:pad.t+h-(p.rate/MAX_SCALE)*h,...p}));
  const tPts=history.map((p,i)=>({x:pad.l+(i/(history.length-1))*w,y:pad.t+h-(getTarget(p.wk||27)/MAX_SCALE)*h}));
...
export default function App() {
  ...
  const addPk = async () => {
    ...
    if (isNaN(r) || r < 0 || r > 40 || !selId || isNaN(w) || w < 1) return;
