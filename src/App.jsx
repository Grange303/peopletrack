...
export default function App() {
  ...
  const openEditEmp = (emp) => {
    ...
    setEditStartDate(emp.startDate || ""); setEditEndDate(emp.endDate || "");
    setShowEditEmp(true);
  };
  const saveEditEmp = async () => {
    if (!sel || !editName.trim() || !editPin.trim()) return;
    await saveEmp({ ...sel, name: editName.trim(), username: editUsername.trim().toLowerCase(), role: editRole.trim() || "Team Member", pin: editPin.trim(), accountType: editAccType, employmentType: editEmpType, startDate: editStartDate, endDate: editEmpType === "seasonal" ? editEndDate : sel.endDate });
